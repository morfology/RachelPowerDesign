...
const SearchResult = ({
  ...
  const generateSearchGroup = (searchResult: ISearchItem[]) => {
    const joinDataByGroup: ISearchGroup[] = searchResult.reduce(
      (groupItems: ISearchGroup[], item: ISearchItem) => {
        const groupIndex = groupItems.findIndex((group) => group.group === item.group);
        if (groupIndex === -1) {
          groupItems.push({
            group: item.group,
            groupItems: [{ frontmatter: { ...item.frontmatter }, slug: item.slug, content: item.content }],
          });
        } else {
  ...
  const matchMarker = (text: string, substring: string) => {
    ...
    return parts.map((part, index) =>
      part.toLowerCase() === substring.toLowerCase() ? <mark key={index}>{part}</mark> : part,
    );
  };
  ...
  const matchContent = (content: string, substring: string) => {
    ...
    while (wordStart > 0 && plainContent[wordStart - 1] !== " ") wordStart--;
    const matches = plainContent.substring(wordStart, substring.length + position);
    const matchesAfter = plainContent.substring(substring.length + position, substring.length + position + 80);
    return (
      <>
          ...
          {finalResult.length > 0 ? (
            finalResult.map((result) => (
              <div className="search-result-group" key={result.group}>
                <p className="search-result-group-title">{titleify(result.group)}</p>
                {result.groupItems.map((item) => (
                  <div key={item.slug} id="searchItem" className="search-result-item">
                    {item.frontmatter.image && (
                      <div className="search-result-item-image">
                        <img src={item.frontmatter.image} alt={item.frontmatter.title} width={100} height={100} />
                      </div>
                    )}
                    <div className="search-result-item-body">
                      <a href={`/${item.slug}`} className="search-result-item-title search-result-item-link">
                      ...
                      {item.content && (
                        <p className="search-result-item-content">{matchContent(item.content, searchString)}</p>
                      )}
                          ...
                          <div className="mr-2">
                            ...
                            {item.frontmatter.categories.map((category, index) => (
                              <span key={category}>
                                ...
                                {item.frontmatter.categories && index !== item.frontmatter.categories.length - 1 && <>, </>}
                              </span>
                            ))}
                          </div>
                        )}
                          ...
                          <div className="mr-2">
                            ...
                            {item.frontmatter.tags.map((tag, index) => (
                              <span key={tag}>
                                ...
                                {item.frontmatter.tags && index !== item.frontmatter.tags.length - 1 && <>, </>}
                              </span>
                            ))}
                          </div>
                        )}
